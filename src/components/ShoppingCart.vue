      ...
      <div class="main">
        <ul>
          <li v-for="cart in carts" :key="cart.id">
            <div class="left">
              <input type="checkbox" class="checkbox" :id="cart.id" />
              <label class="text" :for="cart.id">
                <span>√</span>
              </label>
...
.shoppingcart .list .checkbox:checked + .text {
  background-color: #fdd900;
  border: none;
}
.shoppingcart .list .checkbox:checked + .text > span {
  display: block;
}
